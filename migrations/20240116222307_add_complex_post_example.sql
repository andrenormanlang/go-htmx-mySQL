...
CMSGO is a headless CMS (Content Management System) written in Golang,
...
Perhaps the best feature of CMSGO is its ability to run on very low
...
CMSGO is composed by two different apps:
...
The entire configuration of CMSGO is in a single `toml` file, which you
can pass to both applications with `--config`.

The example config provided in `CMSGO_config.toml` can be used as the
...
CMSGO supports reCaptcha for the contact form. If these configurations are provided,
...
the `CMSGO_config.toml` file for references. But you can currently:

- Create custom links to any link on the internet (including posts and pages in CMSGO).
...
For how to set this up, please refer to `CMSGO_config.toml`.

## Running CMSGO

CMSGO is developed with Golang, so make sure you have a recent enough
version of the compiler and you also follow the instructions in the
following sections.

### Build Requirements

If you\'re runnig CMSGO locally, you should install all the requirements needed
...
We recommend creating a database called `CMSGO` and running the following
command:

```bash
cd migrations
GOOSE_DRIVER="mysql" GOOSE_DBSTRING="root:root@/CMSGO" goose up
```
...
### Building and Running CMSGO

If your platform has support for `Makefiles`, simply call `make`:

```bash
make build
./tmp/CMSGO --config CMSGO_config.toml
```

This will start CMSGO on `http://localhost:8080`. You can change the
configuration by editing the `CMSGO_config.toml` file.
...
This will start two containers: one containing the `CMSGO` app,
...
to startup just the `mariadb` container, then hook CMSGO to your
...
Once the database is up and migrated, you can run CMSGO with your
...
Currently, the architecture of `CMSGO` is still in its early days.
...
![diagram of CMSGO\'s architecture](/static/assets/CMSGO-architecture.png "CMSGO Application Architecture")
...
'This is CMSGO! This is a short guide on how you can build, configure, and run CMSGO.');
-- +goose StatementEnd
